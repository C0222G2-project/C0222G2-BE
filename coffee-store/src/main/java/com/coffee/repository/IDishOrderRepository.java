...
public interface IDishOrderRepository extends JpaRepository<DishOrder, Integer> {
    ...
    @Query(value = " SELECT dishid, name, price, image, time_order as timeOrder " +
            " FROM (SELECT dish_order.dish_id as dishId, dish.name as name, dish.price as price, dish.image as image, sum(quantity) as time_order " +
            " FROM dish_order inner join dish on dish_order.dish_id = dish.id " +
            " GROUP BY  dish_order.dish_id ORDER BY time_order DESC LIMIT 5) as dto_table", nativeQuery = true)
    List<DishMostOrderDTO> get5DishMostOrderDTO();


    @Query(value = " SELECT id,name,creation_date as dateCreate,price,image FROM dish ORDER BY creation_date DESC LIMIT 5", nativeQuery = true)
